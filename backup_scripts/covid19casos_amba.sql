SELECT * FROM (select * from (SELECT * FROM (SELECT REPLACE (residencia_departamento_nombre, 'SIN ESPECIFICAR', 'CABA')as residencia_departamento_nombre,
    ...
    SUM(CASE WHEN fecha_internacion >= (current_date -1) and sexo='M' THEN 1 ELSE 0 END) as imasculino,
    SUM(CASE WHEN fecha_internacion >= (current_date -1) and sexo='F' THEN 1 ELSE 0 END) as ifemenino,
